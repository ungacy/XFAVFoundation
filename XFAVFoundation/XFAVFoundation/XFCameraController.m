...
- (IBAction)confirmBtnFunc:(id)sender {
    ...
    if (self.photoPreviewImageView) {
        ...
        [weakSelf cropWithVideoUrlStr:weakSelf.videoURL
                                start:0
                                  end:weakSelf.currentVideoTimeLength
                           completion:^(NSURL *outputURL, Float64 videoDuration, BOOL isSuccess) {
                               if (isSuccess) {
...
- (void)requestAuthorizationForVideo {
    ...
    if (videoAuthStatus != AVAuthorizationStatusAuthorized) {
        ...
        UIAlertAction *setAction = [UIAlertAction actionWithTitle:@"设置"
                                                            style:UIAlertActionStyleDefault
                                                          handler:^(UIAlertAction *_Nonnull action) {
